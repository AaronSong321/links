...
if make; then
	for linksFile in $(find ./tests/wasm -name "test-*.links" -print); do
		filenameNoExt=${linksFile:0:${#linksFile}-6};
		filenameNoPath=$(basename $filenameNoExt);
		./links -d --client-backend wasm $linksFile --wat-output "$filenameNoExt.wat" && jankscripten compile "$filenameNoExt.notwasm";
